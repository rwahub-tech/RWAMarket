...
const RWAHubKYCABI = [
  ...
  'function setSelfVerifyEnabled(bool enabled) external',
...
export class RWAHubSDK {
  ...
  async revokeKYC(user: string): Promise<void> {
    if (!this.signer) throw new Error('Signer not connected');
    const tx = await this.kycContract.revokeKYC(user);
    await tx.wait();
  }
  ...
  async withdrawPending(): Promise<void> {
    if (!this.signer) throw new Error('Signer not connected');
    const tx = await this.marketplaceContract.withdrawPending();
    await tx.wait();
  }

  async getPendingWithdrawals(account: string): Promise<BigNumber> {
    return this.marketplaceContract.pendingWithdrawals(account);
  }
