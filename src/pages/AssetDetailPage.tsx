...
import { fetchAssetByIdFromApi } from '@/utils/apiAssets';

const AssetDetailPage = () => {
  const { id } = useParams<{ id: string }>();
  const { assets, fetchAssets } = useMockDataStore();
  const [remoteAsset, setRemoteAsset] = useState<any>(null);
  ...
  const asset = remoteAsset || assets.find(a => a.id === id);

  if (!asset) {
    return (
      <div className="container-custom py-16 text-center">
        <h2 className="text-2xl font-bold text-neutral-800 mb-4">
          {loadError ? 'Asset Not Found' : 'Loading asset...'}
        </h2>
        <p className="text-neutral-600 mb-8">
          {loadError
            ? "The asset you're looking for doesn't exist or has been removed."
            : 'Fetching this asset from the API.'}
        </p>
        <Link to="/marketplace">
