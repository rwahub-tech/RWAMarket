...
class CoinGeckoService {
  ...
  async getCoinDetails(coinId) {
    ...
    const response = await axios.get(`${this.baseUrl}/coins/${encodeURIComponent(coinId)}`);
    return response.data;
  }
  ...
  async getHistoricalData(coinId, days = 30, vsCurrency = 'usd') {
    ...
    const response = await axios.get(`${this.baseUrl}/coins/${encodeURIComponent(coinId)}/market_chart`, { params });
    return response.data;
  }
...
class WebhookService {
  ...
  assertSafeWebhookUrl(urlString) {
    let parsed;
    try {
