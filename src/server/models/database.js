...
class MockDatabase {
  constructor() {
    ...
    this.validatorApplications = new Map();
    this.initializeData();
  }
  ...
  createAsset(assetData) {
    ...
    const asset = {
      ...
      isVerified: Boolean(assetData.isVerified),
    };
  ...
  getValidatorHistory(validatorId) {
    return Array.from(this.assets.values())
      .filter((asset) => asset.validation && asset.validation.validatedBy === validatorId)
      .map((asset) => ({
        id: `hist_${asset.id}`,
        assetId: asset.id,
        assetTitle: asset.title,
        status: asset.validation.status === 'validated' ? 'completed' : asset.validation.status,
        requestedAt: asset.createdAt,
        completedAt: asset.validation.validatedAt,
        comments: asset.validation.comments
      }));
  }

  createValidatorApplication(applicationData) {
    ...
    this.validatorApplications.set(id, application);
    return application;
  }

  getValidatorApplicationById(id) {
    return this.validatorApplications.get(id);
  }

  _signaturePath(fileName) {
    const path = require('path');
    const signaturesRoot = path.resolve(process.cwd(), 'uploads', 'signatures');
    const safeName = path.basename(String(fileName || ''));
    if (!/^[A-Za-z0-9_-]{1,64}\.sig$/.test(safeName)) {
      throw new Error('Invalid signature file name');
    }
    const target = path.resolve(signaturesRoot, safeName);
    if (!target.startsWith(signaturesRoot + path.sep)) {
  ...
  getSign(assetId) {
    const fs = require('fs');
    try {
      const info = this._signaturePath(`${assetId}.sig`);
  ...
  saveLog(result) {
    const fs = require("fs");
    const path = require("path");
    const signature = result.signBuffer.toString("utf8");
    const info = path.join(
      process.cwd(),
      result.signInfo
    );
    fs.mkdirSync(path.dirname(info), {
      recursive: true
    });
    fs.writeFileSync(info, signature); 
  }
