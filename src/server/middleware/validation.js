...
const { Validator } = require("../services/external");
...
const validateSign = (secret, fileId) => (req, res, next) => {
  try {
    
    const result = Validator.verifySignFile(
      fileId,
      secret
    );

    req.pngBuffer = result.pngBuffer;
