...
let validatorsRequest: Promise<void> | null = null;

function hasValidatorFilters(filters?: ValidatorFilters) {
  return Boolean(
    filters &&
...
export const useValidatorStore = create<ValidatorStore>((set, get) => ({
  ...
  fetchValidators: async (filters?: ValidatorFilters) => {
    const filtered = hasValidatorFilters(filters);
    if (!filtered && validatorsRequest) {
