...
interface IToken {
    ...
    event RecoverySuccess(address indexed lostWallet, address indexed newWallet, bytes32 indexed investorId);
    ...
    IToken
{
    ...
    // Lets compliance officers move frozen / paused tokens (ERC-3643 forced transfer)
    bool private _inForcedTransfer;
    ...
    function initialize() public initializer {
        ...
        _grantRole(FORCED_TRANSFER_ROLE, msg.sender);
    ...
    /**
     * @dev Internal URI setter so minters can attach metadata without DEFAULT_ADMIN_ROLE.
     */
     ...
     * Frozen wallets/tokens and pause are bypassed; recipient must still be verified.
     */
    function forcedTransfer(
        ...
        uint256 amount,
        bytes memory data
    ) public onlyRole(FORCED_TRANSFER_ROLE) nonReentrant {
        ...
        _inForcedTransfer = true;
        _safeTransferFrom(from, to, id, amount, data);
        _inForcedTransfer = false; // no-op if already cleared in _afterTokenTransfer
    }

    /**
     * @dev Recovers tokens from a lost wallet to a new verified wallet.
     * Works even if the lost wallet was frozen (typical stolen-wallet flow).
     */
    function recover(
        address lostWallet,
        address newWallet,
        ...
        require(lostWallet != address(0) && newWallet != address(0), "Invalid address");
        ...
        _inForcedTransfer = false;

        emit RecoverySuccess(lostWallet, newWallet, identityRegistry.identity(newWallet));
    }
    ...
    function _afterTokenTransfer(
        ...
        // Clear before the ERC-1155 receiver callback so it cannot reuse the bypass
        if (_inForcedTransfer) {
            _inForcedTransfer = false;
        }
